...
export class ALBIngressController extends cdk.Construct {
  constructor(scope: cdk.Construct, id: string, props: ALBIngressControllerProps) {
    ...
    const policyJson = fs.readFileSync("./assets/alb-ingress-controller/iam-policy.json").toString();
    ...
    const rbacRoles = yaml.safeLoadAll(fs.readFileSync("./assets/alb-ingress-controller/rbac-role.yaml").toString())
      .filter((rbac: any) => {
        return rbac["kind"] != "ServiceAccount"
      });

    const albDeployment = yaml.safeLoad(fs.readFileSync("./assets/alb-ingress-controller/alb-ingress-controller.yaml").toString());
